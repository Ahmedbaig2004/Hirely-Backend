...
#         models['calibrator'] = joblib.load("models/isotonic_calibrator_audio.pkl")
...
#             # ✅ Only remove true noise (not real signal)
#             if abs(sv) < NOISE_FLOOR:
#                 continue
...
#         all_pos = [c for c in contributors if c["direction"] == "increased"]
#         all_neg = [c for c in contributors if c["direction"] == "decreased"]

#         # ✅ Always show strengths
#         significant_pos = all_pos[:3]

#         # ✅ Only show meaningful negatives (your “not too strict” control)
#         significant_neg = [
#             c for c in all_neg 
#             if c["impact_magnitude"] >= COACHING_THRESHOLD
#         ][:3]

#         # ✅ CRITICAL: fallback so you NEVER get empty SHAP
#         if not significant_neg and all_neg:
#             significant_neg = [all_neg[0]]
...
#     calibrated_score = float(models['calibrator'].predict([clipped_raw])[0])
#     calibrated_score = float(np.clip(calibrated_score, 0.0, 1.0))

#     logger.info(
#         f"Score pipeline: raw={clipped_raw:.4f} → calibrated={calibrated_score:.4f}"
#     )

#     # 5 — SHAP uses RAW score (model explanation)
...
#     return {
#         "confidence_score": calibrated_score,   # ✅ ONLY THIS
#         "confidence_label": score_to_label(calibrated_score),  # ✅ use calibrated
#         "wpm": wpm,
#         "shap": shap_result,
...
if __name__ == "__main__":
    ...
    print("="*60 + "\n")
    uvicorn.run(app, host="0.0.0.0", port=8001)
>>>>>>> a37e345bdd1aa6c49e1e4c08935351d5987502ed
